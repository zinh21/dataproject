...
try:
    ...
    with tab1:
        ...
        if show_trend:
            ...
            ax.plot(x, trend, color='darkgreen',
                    ...
                    label=f'전체 추세선 (기울기={slope*10:.3f}°C/10년)')
    ...
    with tab2:
        ...
        axes[0].set_ylabel('평균기온 (°C)', fontsize=12)
        ...
        axes[1].axvline(before_vals.mean(), color='steelblue',
                        linewidth=2, linestyle='--',
                        label=f'이전 평균: {before_vals.mean():.1f}°C')
        axes[1].axvline(after_vals.mean(), color='tomato',
                        linewidth=2, linestyle='--',
                        label=f'이후 평균: {after_vals.mean():.1f}°C')
        axes[1].set_xlabel('평균기온 (°C)', fontsize=12)
    ...
    with tab3:
        ...
        ax.axhline(y=before, color='steelblue', linewidth=1.5,
                   linestyle='--', alpha=0.7,
                   label=f'{split_year}년 이전 평균: {before:.2f}°C')
        ax.axhline(y=after, color='tomato', linewidth=1.5,
                   linestyle='--', alpha=0.7,
                   label=f'{split_year}년 이후 평균: {after:.2f}°C')
        ...
        ax.legend(handles=legend_elements +
                  [plt.Line2D([0], [0], color='steelblue', linestyle='--',
                               label=f'이전 평균: {before:.2f}°C'),
                   plt.Line2D([0], [0], color='tomato', linestyle='--',
                               label=f'이후 평균: {after:.2f}°C')],
                  fontsize=10)
    ...
    st.error("⚠️ 'ta_20260601093156.csv' 파일을 찾을 수 없습니다. 앱과 같은 폴더에 파일을 넣어주세요!")
